...
// Configuración por instancia. Permite correr varios bridges a la vez, cada uno
// con su propia cuenta de WhatsApp, sin que choquen entre sí.
//
//	WHATSAPP_STORE_DIR    carpeta de datos (sesión, mensajes, multimedia). Default "store".
//	WHATSAPP_BRIDGE_PORT  puerto del API REST local. Default 8080.
//
// Sin variables definidas se comporta exactamente igual que antes de parametrizar.
var (
	storeDir   = envOr("WHATSAPP_STORE_DIR", "store")
	bridgePort = envPortOr("WHATSAPP_BRIDGE_PORT", 8080)
)
...
func envPortOr(key string, fallback int) int {
	...
	if err != nil || p < 1 || p > 65535 {
		fmt.Fprintf(os.Stderr, "%s invalido (%q), usando %d\n", key, v, fallback)
		return fallback
	}
	return p
}
...
func extractDirectPathFromURL(url string) string {
	...
	// Keep query parameters: WhatsApp's direct_path includes them (ccb/oh/oe/_nc_sid),
	// and whatsmeow appends "&hash=..." assuming a "?" is already present.
	return "/" + pathPart
}
...
func startRESTServer(client *whatsmeow.Client, messageStore *MessageStore, port int) {
	...
	http.HandleFunc("/api/markread", func(w http.ResponseWriter, r *http.Request) {
		...
		for _, chatJID := range req.ChatJIDs {
			res := chatResult{ChatJID: chatJID}

			chat, err := types.ParseJID(chatJID)
			if err != nil {
				res.Error = fmt.Sprintf("bad chat jid: %v", err)
				results = append(results, res)
				continue
			}

			rows, err := messageStore.db.Query(
				`SELECT id, sender, timestamp FROM messages
				 WHERE chat_jid = ? AND is_from_me = 0
				 ORDER BY timestamp DESC LIMIT ?`, chatJID, req.Limit)
			if err != nil {
				res.Error = err.Error()
				results = append(results, res)
				continue
			...
			// Group message IDs by sender; MarkRead only accepts one sender per call.
			...
			if req.DryRun {
				for _, ids := range bySender {
					res.Marked += len(ids)
				}
				totalMarked += res.Marked
				results = append(results, res)
				continue
			}
			...
			for sender, ids := range bySender {
				...
				if isGroup {
					if strings.Contains(sender, "@") {
						...
						if err != nil {
							res.Error = fmt.Sprintf("bad sender jid %s: %v", sender, err)
							continue
						}
					} else {
				...
				if err := client.MarkRead(context.Background(), ids, latest[sender], chat, senderJID); err != nil {
					res.Error = err.Error()
					continue
				}
				res.Marked += len(ids)
